 ...
 * POST /api/nft/mint
 * Mints a new NFT
 * Body: { to: string, tokenURI: string, signer: "owner"|"seller"|"bidder1"|... }
 */
router.post("/mint", async (req: Request, res: Response) => {
  try {
    const { to, tokenURI, signer } = req.body;
    if (!to || !tokenURI || !signer) {
      res.status(400).json({
        error: "Missing required fields: to, tokenURI, signer",
      });
      return;
    }
 ...
 * Body: { to: string, tokenId: number, signer: string }
 */
router.post("/approve", async (req: Request, res: Response) => {
  try {
    const { to, tokenId, signer } = req.body;
    if (!to || tokenId === undefined || !signer) {
      res
        .status(400)
        .json({ error: "Missing required fields: to, tokenId, signer" });
      return;
    }
 ...
 * Body: { operator: string, approved: boolean, signer: string }
 */
router.post("/set-approval-for-all", async (req: Request, res: Response) => {
  try {
    const { operator, approved, signer } = req.body;
    if (!operator || approved === undefined || !signer) {
      res.status(400).json({
        error: "Missing required fields: operator, approved, signer",
      });
      return;
    }
 ...
 * Body: { from: string, to: string, tokenId: number, signer: string }
 */
router.post("/transfer", async (req: Request, res: Response) => {
  try {
    const { from, to, tokenId, signer } = req.body;
    if (!from || !to || tokenId === undefined || !signer) {
      res.status(400).json({
        error: "Missing required fields: from, to, tokenId, signer",
      });
      return;
    }
